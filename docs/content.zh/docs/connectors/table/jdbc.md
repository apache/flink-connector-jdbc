...
| Spanner    | `com.google.cloud`         | `google-cloud-spanner-jdbc` (classifier `single-jar-with-dependencies`) | [下载](https://repo1.maven.org/maven2/com/google/cloud/google-cloud-spanner-jdbc/) |
...
- `base-url`：必填，（不应该包含数据库名）
  ...
  - 对于 Spanner Catalog `base-url` 应为 `"jdbc:cloudspanner:/projects/<project>/instances/<instance>/databases/"` 的格式。
