...
@Internal
public class CrateDBTypeMapper implements JdbcDialectTypeMapper {

    private static final Logger LOG = LoggerFactory.getLogger(CrateDBTypeMapper.class);

    // CrateDB (Postgres) jdbc driver maps several alias to real type, we use real type rather than
    // alias:
    // smallint <=> int2
    // integer <=> int4
    // int <=> int4
    ...
    public DataType mapping(ObjectPath tablePath, ResultSetMetaData metadata, int colIndex)
            throws SQLException {
        String crateDBType = metadata.getColumnTypeName(colIndex);
        ...
        switch (crateDBType) {
            ...
            default:
                throw new UnsupportedOperationException(
                        String.format("Doesn't support CrateDB type '%s' yet", crateDBType));
        }
    }
}
