...
public class TableSplitterEnumeratorBuilder {
    ...
    /**
     * Sets the boundedness reported by the enumerator. Defaults to {@link Boundedness#BOUNDED}: a
     * snapshot read is finite and the job terminates once every chunk has been emitted. Use {@link
     * Boundedness#CONTINUOUS_UNBOUNDED} only if the job must keep running after the snapshot has
     * been fully read. Note that boundedness does not change how splits are computed: chunks are
     * always streamed from a background thread and never materialised all at once.
     */
    public TableSplitterEnumeratorBuilder withBoundedness(Boundedness boundedness) {
        this.boundedness = Objects.requireNonNull(boundedness, "boundedness must be provided");
        return this;
    }
