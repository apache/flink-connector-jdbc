...
class FakeConnectionProvider implements ConnectionProvider {
    ...
    final AtomicInteger createGlobalSnapshotCount = new AtomicInteger();
    final AtomicInteger tableColumnsCalls = new AtomicInteger();
    volatile boolean failCreateGlobalSnapshot;

    @Nullable
    @Override
    public String getGlobalSnapshotId() {
        return createGlobalSnapshotCount.get() > 0 ? "fake-exported-snapshot" : null;
    }

    @Override
    public void createGlobalSnapshot() throws SQLException {
        if (failCreateGlobalSnapshot) {
            throw new SQLException("export failed (test)");
        }
        createGlobalSnapshotCount.incrementAndGet();
    }
    ...
    public Set<TableColumn> getTableColumns(TableId tableId) {
        tableColumnsCalls.incrementAndGet();
