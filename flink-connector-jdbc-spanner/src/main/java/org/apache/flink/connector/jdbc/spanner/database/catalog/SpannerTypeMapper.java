...
public class SpannerTypeMapper implements JdbcCatalogTypeMapper {
    ...
    private static final int SPANNER_TIMESTAMP_PRECISION = 9;
    ...
    public DataType mapping(ObjectPath tablePath, ResultSetMetaData metadata, int colIndex)
            throws SQLException {
        final String spannerType = normalizeTypeName(metadata.getColumnTypeName(colIndex));
    ...
    /**
     * Newer versions of the Spanner JDBC driver return the full type name (e.g. {@code ARRAY<BOOL>}
     * or {@code STRUCT<...>}) instead of only the type code. Strip the type parameters so that the
     * mapping only depends on the type code.
     */
    private static String normalizeTypeName(String spannerType) {
        final int index = spannerType.indexOf('<');
        return index == -1 ? spannerType : spannerType.substring(0, index);
    }
    ...
    private DataType getMapping(
        ...
        switch (spannerType) {
            ...
            case SPANNER_TIMESTAMP:
                // Spanner TIMESTAMP is an absolute instant with nanosecond precision, which
                // corresponds to TIMESTAMP_LTZ rather than TIMESTAMP (like PostgreSQL timestamptz).
                return DataTypes.TIMESTAMP_LTZ(SPANNER_TIMESTAMP_PRECISION);
            case SPANNER_ARRAY:
    ...
    private DataType getArrayMapping(String spannerClassName, int precision, int scale) {
        switch (spannerClassName) {
            ...
            case SPANNER_ARRAY_TIMESTAMP:
                return DataTypes.ARRAY(DataTypes.TIMESTAMP_LTZ(SPANNER_TIMESTAMP_PRECISION));
            default:
                throw new UnsupportedOperationException(
