...
class SpannerDynamicTableSourceITCase extends JdbcDynamicTableSourceITCase
        implements SpannerTestBase {
    ...
    protected TableRow createInputTable() {
        return spannerTableRow(
                ...
                // Spanner only has INT64, so smaller integer types are narrowed on read.
                field("int_col", dbType("INT64"), DataTypes.INT()),
                field("smallint_col", dbType("INT64"), DataTypes.SMALLINT()),
                field("tinyint_col", dbType("INT64"), DataTypes.TINYINT()));
    }
