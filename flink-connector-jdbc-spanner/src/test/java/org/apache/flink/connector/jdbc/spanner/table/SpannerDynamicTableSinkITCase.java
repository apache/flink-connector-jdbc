...
class SpannerDynamicTableSinkITCase extends JdbcDynamicTableSinkITCase implements SpannerTestBase {
    ...
    protected void testUpsert() throws Exception {
        // The Spanner emulator only supports one read-write transaction at a time. Since Spanner
        // JDBC driver 2.32.0, all connections share a single multiplexed session, so concurrent
        // transactions from parallel sink subtasks fail with "Invalid transaction ID" or
        // "Transaction not found" on the emulator. Run the upsert sink with a single subtask.
    ...
    protected String appendOnConflictClause() {
        // Spanner requires primary keys, so Flink 2.3+ (FLIP-558) requires an ON CONFLICT clause
        // when writing insert-only data into the sink tables.
        return isFlip558Enabled() ? " ON CONFLICT DO DEDUPLICATE" : "";
    }
    ...
    protected final TableRow ltzOutputTable =
            spannerTableRow(
                    "LTZ_TABLE",
                    pkField("id", dbType("INT64 NOT NULL"), DataTypes.INT().notNull()),
                    field("ts", dbType("TIMESTAMP"), DataTypes.TIMESTAMP_LTZ(3)));

    @Override
    public List<TableManaged> getManagedTables() {
        List<TableManaged> tables = new java.util.ArrayList<>(super.getManagedTables());
        tables.add(arrayOutputTable);
        tables.add(ltzOutputTable);
        return tables;
    }
    ...
    protected void testReal() throws Exception {
        ...
        tEnv.executeSql(
                        String.format(
                                "INSERT INTO %s SELECT 'a', CAST(1.0 as FLOAT)%s",
                                tableName, appendOnConflictClause()))
                .await();
        ...
        tEnv.executeSql(
                        String.format(
                                "INSERT INTO %s VALUES "
                                        ...
                                        + "(3, CAST(NULL AS ARRAY<BIGINT>), "
                                        + "CAST(NULL AS ARRAY<STRING>), "
                                        + "CAST(NULL AS ARRAY<DECIMAL(10, 2)>))%s",
                                tableName, appendOnConflictClause()))
                .await();
    ...
    @Test
    void testTimestampLtzWithNonUtcDefaultTimeZone() throws Exception {
        ...
        try {
            ...
            String tableName = "ltzSink";
            tEnv.executeSql(ltzOutputTable.getCreateQueryForFlink(getMetadata(), tableName));
            tEnv.executeSql(
                            String.format(
                                    "INSERT INTO %s VALUES (1, TO_TIMESTAMP_LTZ(0, 3))%s",
                                    tableName, appendOnConflictClause()))
                    .await();

            // The stored value must be the epoch instant regardless of the JVM time zone.
            try (Connection conn = getMetadata().getConnection();
                    Statement stmt = conn.createStatement();
                    ResultSet rs =
                            stmt.executeQuery("SELECT ts FROM " + ltzOutputTable.getTableName())) {
                assertThat(rs.next()).isTrue();
                assertThat(rs.getTimestamp(1).toInstant()).isEqualTo(Instant.EPOCH);
            }

            // Reading it back through the connector must return the same instant.
            try (CloseableIterator<Row> it =
                    tEnv.executeSql("SELECT ts FROM " + tableName).collect()) {
                assertThat(it.next().getField(0)).isEqualTo(Instant.EPOCH);
            }
        } finally {
